...
class TestSuperSimpleEnv(unittest.TestCase):
    def test___init__(self):
        r = ssenv.SuperSimpleEnv(IPC_limit=10, ipc_cost_arr=numpy.array([3,4,6]), unit_dict={})
        logger.debug("r:  {}".format(r))
        logger.debug("r.action_space.shape:  {}".format(r.action_space.shape))

    def test_reset(self):
        c = ssenv.SuperSimpleEnv(IPC_limit=10, ipc_cost_arr=numpy.array([3,4,6]), unit_dict={})
        ...
        self.assertEqual(c.observation_space.shape[0], r[0].shape[0])
    ...
    def test_convert_action_integers(self):
        ...
        r = ssenv.convert_action_to_integers(action, 13, ipc_cost)
        ...
        r = ssenv.convert_action_to_integers(action, 29, ipc_cost)
        ...
        action = numpy.array([1., 0., 0.])
        r = ssenv.convert_action_to_integers(action, 58, ipc_cost)

        logger.debug("######################")
        logger.debug("IPC limit low")
        action = numpy.array([1., 0., 0., 0., 0.])
        ipc_cost = numpy.array([3, 4, 6, 10, 12])
        r = ssenv.convert_action_to_integers(action, 10, ipc_cost)
        logger.debug("IPC limit low - r:  {}".format(r))

    def test_render(self):
        r = ssenv.SuperSimpleEnv(IPC_limit=10, ipc_cost_arr=numpy.array([3,4,6,10,12]), unit_dict={})
        r.render()
